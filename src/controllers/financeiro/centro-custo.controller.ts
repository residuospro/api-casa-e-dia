...
import { centroCustoService } from '../../services/financeiro/centro-custo.service';
import { familyRepository } from '../../repositories/family.repository';
import { AuthRequest } from '../../middlewares/auth.middleware';
import { criarCentroCustoSchema, atualizarCentroCustoSchema } from '../../validators/financeiro.validator';
import { ZodError } from 'zod';
import { parseListagemQuery } from '../../helpers/listagem.helper';

export const centroCustoController = {
  async criar(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await centroCustoService.criar(familiaId, dados);
  ...
  async listar(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await centroCustoService.listar(familiaId, options, params, filtro, ordenacao);
      res.json(resultado);
    } catch (err) {
      next(err);
    }
  ...
  async obter(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await centroCustoService.obter(familiaId, id);
      res.json(resultado);
    } catch (err) {
      next(err);
    }
  ...
  async atualizar(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await centroCustoService.atualizar(familiaId, id, dados);
      res.json(resultado);
    } catch (err) {
      if (err instanceof ZodError) {
        res.status(400).json({ error: 'Bad Request', message: err.errors.map((e) => e.message) });
  ...
  async remover(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      await centroCustoService.remover(familiaId, id);
      res.status(204).send();
    } catch (err) {
      next(err);
    }
