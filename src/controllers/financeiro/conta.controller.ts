...
import { criarContaSchema, atualizarContaSchema } from '../../validators/financeiro.validator';
...
export const contaController = {
  async criar(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await contaService.criar(familiaId, dados);
  ...
  async listar(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await contaService.listar(familiaId, options, params, filtro, ordenacao);
      res.json(resultado);
    } catch (err) {
      next(err);
    }
  ...
  async obter(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      const resultado = await contaService.obter(familiaId, id);
      res.json(resultado);
    } catch (err) {
      next(err);
    }
  ...
  async atualizar(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const { familiaId, id } = req.params;
      const dados = atualizarContaSchema.parse(req.body);
      ...
      const resultado = await contaService.atualizar(familiaId, id, dados);
      res.json(resultado);
    } catch (err) {
      if (err instanceof ZodError) {
        res.status(400).json({ error: 'Bad Request', message: err.errors.map((e) => e.message) });
  ...
  async remover(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      ...
      await contaService.remover(familiaId, id);
      res.status(204).send();
    } catch (err) {
      next(err);
    }
